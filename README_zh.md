...
LLM Proxy 是一个轻量级、中立、开源的 LLM 代理工具，专注于请求拦截与 Token 统计。
...
- 📝 **本地日志**: 
    - `requests.log`: 包含 Token 的实时监控日志（默认开启）。
...
**交互控制：**
- 按 **回车键** → 显示全局统计信息（成功/失败请求次数、总token消耗量、平均响应时间）
- 2秒内再次按 **回车键** → 优雅退出程序
- 等待超过2秒 → 下次按回车显示最新统计
...
有关如何使用代理日志进行对账的详细指引，请参阅：

- [对账与审计指引](./RECONCILIATION_GUIDE_zh.md)
- [Reconciliation Guide (English)](./RECONCILIATION_GUIDE.md)

## 🚨 免责声明

本工具仅供技术研究、个人学习以及对账使用。用户在使用过程中产生的 API 调用费用及遵守目标商服务条款的责任由用户自行承担。严禁用于任何非法用途。
